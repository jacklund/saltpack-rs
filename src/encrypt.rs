...
pub fn decrypt_payload(
    ...
    // have in our keyring
    let mut secret_key_list: Vec<&SecretKey> = header
        .recipients_list
        .iter()
    ...
    let key_boxes: Vec<Vec<u8>> = header
        .recipients_list
        .iter()
    ...
    let num_anon_receivers: usize = header.recipients_list.len() - named_receivers.len();
...
fn process_packet(
    ...
    if authenticator != packet.authenticators[recipient_index as usize] {
